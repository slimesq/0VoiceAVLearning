...
int main(int argc, char** argv)
{
    ...
    int data_size = fread(inbuf, AUDIO_INBUF_SIZE, 1, infile);
    while (1)
    {
        int parseSize = av_parser_parse2(parser,
                                         codec,
                                         &pkt->data,
                                         &pkt->size,
                                         inbuf,
        ...
        int readSize = fread(inbuf + data_size, AUDIO_INBUF_SIZE - data_size, 1, infile);
        data_size += readSize;
    }
